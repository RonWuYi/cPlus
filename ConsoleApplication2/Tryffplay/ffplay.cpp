...
enum class MY_TIME_BASE
{
    MY_AV_TIME_BASE = 1000000,
} MY_TIME_BASE;
static const AVRational MY_AV_TIME_BASE_Q = { 1, (int)MY_TIME_BASE::MY_AV_TIME_BASE };
...
// AVRational
//enum class MyAVRational
//{
//    MY_AV_TIME_BASE_Q = { 1, AV_TIME_BASE },
//
//} MyAVRational;

enum class MY_SWS_BICUBIC
{
    MY_AV_SWS_BICUBIC = 4,
} MY_SWS_BICUBIC;

static unsigned sws_flags = (unsigned int)MY_SWS_BICUBIC::MY_AV_SWS_BICUBIC;
...
static inline
int cmp_audio_fmts(enum class AVSampleFormat fmt1, int64_t channel_count1,
    enum class AVSampleFormat fmt2, int64_t channel_count2)
{
...
static void get_sdl_pix_fmt_and_blendmode(int format, Uint32* sdl_pix_fmt, SDL_BlendMode* sdl_blendmode)
{
    ...
    *sdl_pix_fmt = SDL_PIXELFORMAT_UNKNOWN;
    if (format == AV_PIX_FMT_RGB32 ||
        ...
        *sdl_blendmode = SDL_BlendMode::SDL_BLENDMODE_BLEND;
        ...
        if (frame_queue_nb_remaining(&is->subpq) > 0) {
            ...
            if (vp->pts >= sp->pts + ((float)sp->sub.start_display_time / 1000)) {
                if (!sp->uploaded) {
                    ...
                    if (realloc_texture(&is->sub_texture, SDL_PIXELFORMAT_ARGB8888, sp->width, sp->height, SDL_BlendMode::SDL_BLENDMODE_BLEND, 1) < 0)
                        return;
...
static void video_audio_display(VideoState* s)
{
    ...
    if (!s->paused) {
        ...
        if (s->show_mode == ShowMode::SHOW_MODE_WAVES) {
            h = INT_MIN;
    ...
    if (s->show_mode == ShowMode::SHOW_MODE_WAVES) {
        ...
        if (realloc_texture(&s->vis_texture, SDL_PIXELFORMAT_ARGB8888, s->width, s->height, SDL_BlendMode::SDL_BLENDMODE_NONE, 1) < 0)
            return;
        ...
        if (!s->rdft || !s->rdft_data) {
            ...
            s->show_mode = ShowMode::SHOW_MODE_WAVES;
        }
        else {
            FFTSample* data[2];
...
static void video_display(VideoState* is)
{
    ...
    if (is->audio_st && is->show_mode != ShowMode::SHOW_MODE_VIDEO)
...
static void video_refresh(void* opaque, double* remaining_time)
{
    ...
    if (!display_disable && is->show_mode != ShowMode::SHOW_MODE_VIDEO && is->audio_st) {
    ...
    display:
        /* display picture */
        if (!display_disable && is->force_refresh && is->show_mode == ShowMode::SHOW_MODE_VIDEO && is->pictq.rindex_shown)
            video_display(is);
    }
    is->force_refresh = 0;
    if (show_status) {
...
static int audio_thread(void* arg)
{
    ...
    do {
        ...
        AVRational tmp1 = { 1, frame->sample_rate };
        if (got_frame) {
            tb = tmp1;
...
static int audio_decode_frame(VideoState* is)
{
    ...
    if (af->frame->format != (int)is->audio_src.fmt ||
...
static void toggle_audio_display(VideoState* is)
{
    ...
    } while (next != (int)is->show_mode && (next == (int)ShowMode::SHOW_MODE_VIDEO && !is->video_st || next != (int)ShowMode::SHOW_MODE_VIDEO && !is->audio_st));
    if (is->show_mode != (ShowMode)next) {
        is->force_refresh = 1;
...
static void seek_chapter(VideoState* is, int incr)
{
    ...
    for (i = 0; i < is->ic->nb_chapters; i++) {
        ...
        /*if (av_compare_ts(pos, (AVRational)AV_TIME_BASE_Q, ch->start, ch->time_base) < 0) {*/
...
int main(int argc, char* argv[])
{
    ...
    std::string my_file = "C:\\Work\\media\\NPO1.ts";
    //if (argc < 2)
    //{
    //    show_usage();
