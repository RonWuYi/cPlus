...
static void video_image_display(VideoState* is)
{
    ...
    SDL_RendererFlip tmp_flip_v = static_cast<SDL_RendererFlip>(vp->flip_v ? SDL_FLIP_VERTICAL : 0);
    SDL_RenderCopyEx(renderer, is->vid_texture, NULL, &rect, 0, NULL, tmp_flip_v);
...
#endif
            AVRational tmp = { frame_rate.den, frame_rate.num };
    ...
    if (af->frame->format != is->audio_src.fmt ||
        ...
        if (!is->swr_ctx || swr_init(is->swr_ctx) < 0) {
            av_log(NULL, AV_LOG_ERROR,
                ...
                af->frame->sample_rate, av_get_sample_fmt_name(static_cast<c>(af->frame->format)), af->frame->channels,
                is->audio_tgt.freq, av_get_sample_fmt_name(static_cast<c>(is->audio_tgt.fmt)), is->audio_tgt.channels);
            swr_free(&is->swr_ctx);
            return -1;
        }
